...
SUPPORTED_EXTS = {".mp3", ".wav", ".ogg", ".flac", ".aiff", ".m4a"}
...
def get_devices(direction: str) -> dict[int, str]:
    """Return (index, name) pairs for all input or output devices."""
...
class PlaybackEngine:
    def __init__(self):
        ...
        self._master_stream: sd.OutputStream | None = None
    ...
    def play(self, path: str) -> None:
        ...
        self._master_stream = sd.OutputStream(
            ...
            finished_callback=self._on_master_finished,
        )
    ...
    def _push_buffer(self, chunk: np.ndarray) -> None:
        ...
        while self._buffer_frames > MAX_BUF_FRAMES:
...
class SoundPadApp(tk.Tk):
    ...
    def _build_device_row(self):
        ...
        self.device_lbl = tk.Label(
            row, text="Auto-detected", font=("Courier", 8), fg=TEXT_SEC, bg=DARK_BG
        )
    ...
    def _build_controls(self):
        ...
        tk.Label(
            bar, text="VOL", font=("Courier", 8, "bold"), fg=TEXT_SEC, bg=DARK_BG
        ).pack(side="left")
        ...
        self.vol_lbl = tk.Label(
            bar, text="100%", font=("Courier", 9), fg=TEXT_PRI, bg=DARK_BG, width=5
        )
    ...
    def _build_now_playing(self):
        ...
        tk.Label(
            bar, text="NOW PLAYING:", font=("Courier", 8), fg=TEXT_SEC, bg=PANEL_BG
        ).pack(side="left", padx=12)

        self.np_lbl = tk.Label(
            bar, text="—", font=("Courier", 9, "bold"), fg=ACCENT, bg=PANEL_BG
        )
    ...
    def _refresh_devices(self):
        ...
        self.engine.mic_device = mic
        self.engine.monitor_device = monitor

        if mic is None:
            self.device_lbl.configure(text="No output device found", fg=ACCENT2)
            return

        names = get_devices("output")
